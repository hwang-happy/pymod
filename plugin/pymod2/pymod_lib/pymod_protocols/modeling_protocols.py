...
#       - order the chains of the template complex.
#   - simplify the code for importing the output in PyMod/PyMOL.
...
class MODELLER_homology_modeling(PyMod_protocol, Modeling_session):
    ...
    def _perform_modelization(self):
        ...
        for model_file_number, model in enumerate(a.outputs):
            ...
            if self.superpose_to_templates:
                ...
                if not self.multiple_chain_mode:
                    ...
                    for mod_e in model_chain_elements:
                        model_selector = mod_e.get_pymol_object_name()
        ...
        #     list_of_models_names.append(model['name'])
        #     model_file_full_path = os.path.join(self.modeling_directory, model['name'])
        #     model_file = open(model_file_full_path, "r")
        #     obj_funct_value = float(model_file.readlines()[1][39:].replace(" ",""))
        #     model_file.close()
        #     # Gets the DOPE values.
        #     model_profile_shortcut = os.path.join(self.modeling_directory, model['name'][:-4]+".profile")

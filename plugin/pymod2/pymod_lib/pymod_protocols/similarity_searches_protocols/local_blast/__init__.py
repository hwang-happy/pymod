...
class LOC_BLAST_common:
    ...
    def execute_loc_blast(self, ncbi_dir, db_path, query,
                               #inclusion_ethresh=0.001, num_iterations=3,
                               evalue=None,max_target_seqs=None, out=None, outfmt=None): #num_alignments = None,  outfmt=None, out_pssm=None):
        """
        Execute the locally installed BLAST. Used when running BLAST through the 'Local BLAST'
        command on the plugin main menu.
        """
        ...
        try:
            ...
            locblast_command = self.build_psiblast_commandline(
                ncbi_dir = ncbi_dir,
                db_path = dp_prefix,
            ...
            # Execute BLAST.
            self.pymod.execute_subprocess(locblast_command)
        ...
        except ZeroDivisionError: #todo errore a caso
            ...
            if os.path.isdir(os.path.join(db_path, temp_output_dir_name)):
                shutil.rmtree(os.path.join(db_path, temp_output_dir_name))
            raise Exception("There was some error while running BLAST with the input query: %s." % (query))
    ...
    def build_psiblast_commandline(self, ncbi_dir, db_path, query,
        ...
        locblast_path = pymod_os_specific.build_commandline_path_string(os.path.join(ncbi_dir, pymod_os_specific.get_exe_file_name("blastp")))
        ...
        locblast_command = (locblast_path + db_path + query + out + outfmt +
                            #inclusion_ethresh + out + outfmt + out_pssm + num_iterations +
                            evalue + max_target_seqs) # + num_alignments)

        print(locblast_command)
        return locblast_command
...
class LOC_BLAST_search(Generic_BLAST_search, LOC_BLAST_common):
    ...
    def run_psiblast(self):
        ...
        try:
            ...
            self.blast_options_window.show_error_message(
                "BLASTp Error",
                "There was an error while running BLASTp for %s." % (self.blast_query_element.my_header))
            return False

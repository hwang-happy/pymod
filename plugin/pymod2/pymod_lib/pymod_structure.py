...
class Associate_structure:
    ...
    def __init__(self, pdb_file_path, chain_id, pymod_element, output_directory=""):

        self.target_element = pymod_element
        self.structure_chain_id = chain_id
        self.output_directory = output_directory
        self.original_pdb_file_path = pdb_file_path
        # Parses the source structure file.
        p = Parsed_pdb_file(pdb_file_path, output_directory=output_directory, copy_original_file=False, save_chains_files=False)
        self.structure_chain_element = p.get_pymod_element_by_chain(self.structure_chain_id)
    ...
    def associate(self):
        ...
        cmd.select(self.temp_fragment_name, "resi %s-%s and object %s and not hetatm" % (start_position, end_position, self.temp_full_name))
        ...
        cmd.save("%s_cropped.pdb" % (cropped_structure_file_shortcut), self.temp_fragment_name)
...
def associate_structure(pdb_file_path, chain_id, pymod_element, output_directory=""):
    a = Associate_structure(pdb_file_path, chain_id, pymod_element, output_directory=output_directory)
    a.associate()
